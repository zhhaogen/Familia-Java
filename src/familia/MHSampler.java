 ...
 */
public class MHSampler implements Sampler
{
	public static class TopicIndex extends ArrayList<Integer>
	{
		/** */
	...
	int construct_alias_table()
	{
		...
		_topic_indexes = Lists.newArrayList(vocab_size, TopicIndex.class);
		_alias_tables = Lists.newArrayList(vocab_size, VoseAlias.class);
		_prob_sum = Lists.newArrayList(vocab_size, 0d);
		...
		for (int i = 0; i < vocab_size; ++i)
		{
			dist.clear();
			double prob_sum = 0;
			for (TopicCount iter : _model.word_topic(i))
			{
				...
				{
					TopicIndex ele = _topic_indexes.get(i);
					ele.add(topic_id);
					_topic_indexes.set(i, ele);
				}
		...
		_beta_prior_sum = 0;
		List<Double> beta_dist = new ArrayList<>(_model._num_topics);
		...
		Logger.d("#beta_prior_sum=" + _beta_prior_sum  );
		return 0;
	}
