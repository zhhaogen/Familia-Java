 ...
 */
public class SimpleTokenizer  implements familia.java.inter.Constant, Tokenizer
{ 
	// 词表中单词最大长度
	int _max_word_len;
	// 词典数据结构
	...
	void load_vocab(String vocab_path)
	{
		...
		try
		{
			...
			Logger.debug("总共词数:" + _vocab.size()+",词最大长度:"+_max_word_len);
		} catch (IOException e)
		{
			e.printStackTrace();
		}
